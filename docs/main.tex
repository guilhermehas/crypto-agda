...
The current version of Agda is Agda 2.
  ...
  This is another example of \emph{Data Set}, but it depend on the argument.
  ...
  In case of the function \emph{id}, the type of the input can be deduced by the compiler.
...
the amount spent, its message, its signature and its public key.
...
and a proof that this vector is the same of the list of outputs of this type.
...
It has to validate first that the \emph{list of outputs} are a valid \emph{Vector Output}.
...
The another case, it is done in a recursive way.
...
is the same of the message of the \emph{vector output}.
In case that \emph{vector output} has just size one or two, it is a trivial case.
The other cases are doing it in a recursive way.

\emph{sigPub} is another function that returns a proof that a input message is really signed.
...
The list has to be at least with size one.
...
Another validation is verifying if the amount spent of inputs are greater than the amount of the outputs.
The function \emph{Signed?}, defined in the cripto library, validates if the
message was really signed with the input.
...
Here, a function that add a transaction to a transaction tree.
...
If it can, it validates if the amount of vector output is equal total fees plus the block reward.
After, it validates if the time of the transaction is equal to the time of the transaction tree.
In the end, it add the outputs of the transaction to the vector of outputs.
Because it is a coinbase transaction, there is no inputs to be removed.

\agda{RawTXTree}{addtxtree2}

The second case, that the transaction is a regular transaction, looks like the same.
First, it validate if the quantity of transaction is less than the maximum allowed.
Second, it validate if this transaction is a valid signed transaction.
...
minus the inputs.
In case of invalid transaction, it returns nothing.

\subsection{Proofs}

One of the importants proofs is that the each output of \emph{outputs transaction} is distinct.
This is very important, because it garantees that each input in the transaction could be
...
The union of distinct lists make a new distinct list if both are distinct to each other.

Now, to proof that outputs are a distict list:

\agda{proofsTXTree}{uniqueouts}

In the first case, the transaction tree is a genesis tree without any outputs.
So a empty list is a distinct list.
In the second case, the outputs are the union of inputs of the transacion with the outputs
of \emph{vector output}.
So, it is necessary to proof that inputs of the transaction are distinct,
...
There are some cases to proof that inputs are distinct.
...
the number of inputs are zero.
So they are distinct.

In the other cases, it does the same thing of the proof of unique outputs.
...
Both are distinct to each other because all of the transactions of input has the time less
than the time of the transaction.
And because all of outputs of the currenct transaction has time equal to the current time
of this transaction.

\agda{proofsTXTree}{outtimeless}

The proof that the time of the outputs is less than current time of the transaction
...
Ledger in the cripto currency is like a wallet.
It makes it easier for users to send their coins or to know how much money they have in total.

Here, the definition of how much money the user have in the last tree:
...
If there is no output, it returns zero of amount.
If there is at least one output, it verifies if the output adress is the same of the address.
If it is, it add the amount to the amount of the rest of the outputs.
If it is not, it just return the result of recursion of the rest of the outputs.
...
It is not always possible to generate a block from transaction tree.
It is because, the last transaction of transaction tree must be a coinbase transaction.
Here, the function that returns a decidable if it is possible to generate a block from
the transaction tree.

\agda{Blockchain}{treeblock}

The definition of raw block get just the coinbase transaction tree as explicit type.
...
The definition of coinbase tree is the one that the last transaction is a coinbase.

The code verify if the last transaction tree is a coinbase tree:

\agda{Blockchain}{iscoinbase}

If it is, it return that it is possible to create a block from that with the block definition.
If it is not, it return that it is impossible to create a block from this transaction tree.
...
The defintion of \emph{fstTree} is that it has a tree that is before this tree in the type.
...
If this tree is not, it has to verify if the block number of last tree is the same of this tree.
If the block number is equal, it can find the first tree in a recursive way.
If it is not, it has to provide a proof that it is impossible that this tree is not
...
This text has showed the definition of Bitcoin protocol.
Definitions of transactions, transactions tree, block and blockchain.
In this work, it was also made the code that transform raw transactions in a possible valid transaction,
to incorporate it in the transaction tree.

\subsection{Future work}

In this work, there was a code that transform a raw transaction in a possible valid transaction.
...
There is no definition of cripto functions like SHA-256 and elliptic curves in this work.
...
So it is not possible to add transactions in the blockchain from command line or from the network.

The criptocurrency of this work does not have any smart contract.
